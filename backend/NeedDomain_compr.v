...
Ltac custom_tac38 H0 := unfold H0; intros.

Ltac custom_tac18  := intros; red; intros.

Ltac custom_tac42 H0 := simpl in *; auto; unfold H0.

Ltac custom_tac43 H0 := InvAgree; apply H0.

Ltac custom_tac63 H0 H1 H2 H3 := simpl; intros; rewrite H0; intros [ H1 H1]; eelim H3; eauto.

Ltac custom_tac47 H0 H1 H2 H3 := red; intros; autorewrite with ints; try lia; destruct ( zlt H0 H0) as [ H1 | H1].

Ltac custom_tac7 H0 H1 H2 H3 := auto; destruct H0 as [ | H1 | H1 | H1 | H1 | H1 H1].

Ltac custom_tac10  := autorewrite with ints; auto.

Ltac custom_tac12 H0 := rewrite H0; auto.

Ltac custom_tac37 H0 := intros; apply H0.

Ltac custom_tac39 H0 := f_equal; apply H0.

Ltac custom_tac50 H0 H1 := inv H0; rewrite H1.

Ltac custom_tac67 H0 := split; apply H0.

Ltac custom_tac64 H0 := intros; inversion H0; subst.

Ltac custom_tac1  := simpl; auto.

Ltac custom_tac8  := red; auto.

Ltac custom_tac45 H0 := auto; apply H0.

Ltac custom_tac5  := intros; constructor.

Ltac custom_tac15  := constructor; intros.

Ltac custom_tac34 H0 := eapply H0; eauto.

Ltac custom_tac51  := auto; auto.

Ltac custom_tac52  := simpl; intros.

Ltac custom_tac61 H0 := eelim H0; eauto.
...
Proof. induction vl1; intros; inv H; constructor; auto with na. Qed.
...
Proof. unfold nlub; destruct x, y; auto with na. constructor. intros. autorewrite with ints; auto. rewrite H0; auto. Qed.

Lemma nge_lub_r:
  forall x y, nge (nlub x y) y.
Proof. unfold nlub; destruct x, y; auto with na. constructor. intros. autorewrite with ints; auto. rewrite H0. apply orb_true_r; auto. Qed.
...
Proof. unfold shrimm; intros. unfold Val.shr. destruct (Int.ltu n Int.iwordsize). destruct x; simpl in *. - auto. - InvAgree. destruct (Int.eq_dec (Int.shru (Int.shl m n) n) m). apply iagree_shr_1; auto. apply iagree_shr; auto. - InvAgree. apply Val.lessdef_same. f_equal. apply iagree_mone. apply iagree_shr. auto. - destruct v; auto with na. Qed.
...
Proof. destruct nv; simpl; auto. f_equal; apply complete_mask_idem. Qed.
...
Proof. intros; red; intros. rewrite PMap.gsspec. destruct (peq r0 r); auto. subst. unfold nreg in H. rewrite H. red; auto. Qed.
...
Lemma nlive_lub_r:
  forall nm1 nm2 b i, nlive nm2 b i -> nlive (nmem_lub nm1 nm2) b i. Proof. intros. inversion H; subst. destruct nm1; simpl. auto. constructor; simpl; intros. - rewrite ISet.In_inter. intros [P Q]. eelim STK; eauto. - rewrite PTree.gcombine in H1 by auto. destruct gl0!id as [iv1|] eqn:NG1; try discriminate;
...
Proof. unfold eq; destruct x; simpl; split. apply NE.eq_refl. tauto. Qed.

Lemma eq_sym: forall x y, eq x y -> eq y x.
Proof. unfold eq; destruct x, y; simpl. intros [A B]. split. apply NE.eq_sym; auto. intros. rewrite B. tauto. Qed.

Lemma eq_trans: forall x y z, eq x y -> eq y z -> eq x z.
Proof. unfold eq; destruct x, y, z; simpl. intros [A B] [C D]; split. eapply NE.eq_trans; eauto. intros. rewrite B; auto. Qed.

Definition beq (x y: t) : bool :=
    NE.beq (fst x) (fst y) && nmem_beq (snd x) (snd y).

Lemma beq_correct: forall x y, beq x y = true -> eq x y.
Proof. unfold beq, eq; destruct x, y; simpl; intros. InvBooleans. split. apply NE.beq_correct; auto. intros. apply nmem_beq_sound; auto. Qed.
...
Lemma ge_refl: forall x y, eq x y -> ge x y.
Proof. unfold eq, ge; destruct x, y; simpl. intros [A B]; split. apply NE.ge_refl; auto. intros. apply B; auto. Qed.

Lemma ge_trans: forall x y z, ge x y -> ge y z -> ge x z.
Proof. unfold ge; destruct x, y, z; simpl. intros [A B] [C D]; split. eapply NE.ge_trans; eauto. auto. Qed.

Definition bot : t := (NE.bot, NMemDead).

Lemma ge_bot: forall x, ge x bot.
Proof. unfold ge, bot; destruct x; simpl. split. apply NE.ge_bot. intros. inv H. Qed.

Definition lub (x y: t) : t :=
    (NE.lub (fst x) (fst y), nmem_lub (snd x) (snd y)).

Lemma ge_lub_left: forall x y, ge (lub x y) x.
Proof. unfold ge; destruct x, y; simpl; split. apply NE.ge_lub_left. intros; apply nlive_lub_l; auto. Qed.

Lemma ge_lub_right: forall x y, ge (lub x y) y.
Proof. unfold ge; destruct x, y; simpl; split. apply NE.ge_lub_right. intros; apply nlive_lub_r; auto. Qed.
