...
Ltac custom_tac23 H0 := eauto with rtlg; eapply H0; eauto with rtlg.

Ltac custom_tac8 H0 := intros; monadInv H0.

Ltac custom_tac9 H0 := eapply H0; eauto.

Ltac custom_tac17 H0 H1 H2 H3 H4 := generalize H0; unfold H1; caseEq ( nth_error ( map_letvars H2) H2); intros; inv H4.

Ltac custom_tac4 H0 := left; exists H0; auto.

Ltac custom_tac0 H0 H1 := intros until H0; unfold H1.

Ltac custom_tac6 H0 := apply H0; eauto with rtlg.

Ltac custom_tac18  := econstructor; eauto.

Ltac custom_tac21  := econstructor; eauto with rtlg.

Ltac custom_tac25 H0 H1 := exploit transl_exit_charact; eauto; intros [ H0 H0].

Ltac custom_tac26  := constructor; constructor; eauto with rtlg.

Ltac custom_tac3 H0 := intros; inv H0.

Ltac custom_tac1 H0 := exists H0; auto.

Ltac custom_tac10  := simpl; intros.

Ltac custom_tac12  := constructor; auto.

Ltac custom_tac16  := eauto with rtlg; eauto with rtlg.
...
Proof. intros. monadInv H. split. eauto with rtlg. inversion EQ. subst. red. intros r' [[id A] | B]. simpl in A. rewrite PTree.gsspec in A. destruct (peq id name). inv A. eauto with rtlg. apply reg_valid_incr with s1. eauto with rtlg. apply H0. left; exists id; auto. simpl in B. apply reg_valid_incr with s1. eauto with rtlg. apply H0. right; auto. Qed.
...
Proof. induction namel; simpl; intros; monadInv H. split. red; simpl; intros; tauto. auto. exploit IHnamel; eauto. intros [A B]. exploit add_var_valid; eauto. intros [C D]. split. apply regs_valid_cons; eauto with rtlg. auto. Qed.
...
Lemma add_vars_letenv:
  ...
  map2.(map_letvars) = map1.(map_letvars).
Proof. induction il; simpl; intros; monadInv H. reflexivity. transitivity (map_letvars x0). eapply add_var_letenv; eauto. eauto. Qed.
...
Lemma alloc_regs_valid:
  ...
  regs_valid rl s2.
Proof. induction al; simpl; intros; monadInv H0. apply regs_valid_nil. apply regs_valid_cons. eauto with rtlg. eauto with rtlg. Qed.
...
Proof. induction al; simpl; intros; monadInv H0. elim H1. elim H1; intro. subst r. eapply alloc_reg_fresh_or_in_map; eauto. exploit IHal. 2: eauto. apply map_valid_incr with s; eauto with rtlg. eauto. intros [A|B]. auto. right; eauto with rtlg. Qed.
...
Proof. induction al; intros; monadInv H1. constructor. constructor. eapply alloc_reg_target_ok; eauto. apply IHal with s s2 INCR1; eauto with rtlg. apply regs_valid_cons; eauto with rtlg. Qed.
...
Lemma convert_builtin_res_charact:
  forall map oty res s res' s' INCR
    ...
    (WF: map_valid map s),
  tr_builtin_res map res res'.
Proof. destruct res; simpl; intros. - monadInv TR. constructor. unfold find_var in EQ. destruct (map_vars map)!x; inv EQ; auto. - destruct (xtype_eq oty Xvoid); monadInv TR. + constructor. + constructor. eauto with rtlg. - monadInv TR. Qed.
